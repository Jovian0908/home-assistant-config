...
def gen_family_manual(ha: dict, mesh_tools: list[dict]) -> str:
    ...
    if lights:
        for light in sorted(lights, key=lambda x: x["entity_id"])[:20]:
            fn = light.get("attributes", {}).get("friendly_name", light["entity_id"])
            state = light["state"]
    ...
    if locks:
        for lock in locks:
            fn = lock.get("attributes", {}).get("friendly_name", lock["entity_id"])
            lines.append(f"- **{fn}** - {lock['state']}")
    else:
...
def gen_technical_manual(ha: dict, mesh_tools: list[dict]) -> str:
    ...
    if unavail:
        lines.append(
            f"_{len(unavail)} entities currently unavailable. {len(unavail)/max(1,len(states))*100:.1f}% of total._"
        )
        lines.append("")
        lines.append("Top 30:")
        lines.append("")
