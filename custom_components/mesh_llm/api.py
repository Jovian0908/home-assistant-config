...
async def warm_http() -> None:
    ...
    if _client is None:
        ...
        _client = await loop.run_in_executor(None, lambda: httpx.AsyncClient(timeout=httpx.Timeout(60.0, connect=5.0)))
...
class MeshCouncilDecide(llm.Tool):
    ...
    parameters = vol.Schema(
        {
            ...
            vol.Optional(
                "depth",
                default="normal",
                description="fast = single planner only; normal = full chain; deep = debate triplet.",
            ): vol.In(["fast", "normal", "deep"]),
        }
    )
    ...
    async def async_call(
        ...
        try:
            r = await _http().post(
                f"{COUNCIL_URL}/council/ask",
                json={"query": tool_input.tool_args["query"], "depth": tool_input.tool_args.get("depth", "normal")},
                headers=HEADERS,
            )
            r.raise_for_status()
            data = r.json()
    ...
    parameters = vol.Schema(
        {
            ...
            vol.Optional("k", default=5, description="Number of results (1-20)."): vol.All(
    ...
    async def async_call(
        ...
        try:
            r = await _http().post(
                ...
                json={"query": tool_input.tool_args["query"], "n_results": tool_input.tool_args.get("k", 5)},
                headers=HEADERS,
            )
            r.raise_for_status()
            return r.json()
...
class MeshKnowledgeIngest(llm.Tool):
    ...
    async def async_call(
        ...
        try:
            ...
            r = await _http().post(f"{KNOWLEDGE_URL}/ingest/conversations", json=payload, headers=HEADERS)
            r.raise_for_status()
            return r.json()
        except Exception as e:
...
class MeshSentimentAnalyze(llm.Tool):
    ...
    async def async_call(
        ...
        try:
            r = await _http().post(
                f"{SENTIMENT_URL}/analyze", json={"text": tool_input.tool_args["text"]}, headers=HEADERS
            )
            r.raise_for_status()
            return r.json()
        except Exception as e:
    ...
    async def async_call(
        ...
        try:
            r = await _http().get(
                ...
                params={"q": tool_input.tool_args["query"], "n": tool_input.tool_args.get("n", 5)},
                headers=HEADERS,
            )
            r.raise_for_status()
            return r.json()
...
class MeshVisionListRecent(llm.Tool):
    ...
    parameters = vol.Schema(
        {
            vol.Optional("camera", description="Filter to one camera or omit for all."): vol.In(
                ["aqara_g3", "tapo_c260", "g410_front"]
            ),
        }
    )
    ...
    async def async_call(
        ...
        cams = (
            [tool_input.tool_args["camera"]]
            if "camera" in tool_input.tool_args
            else ["aqara_g3", "tapo_c260", "g410_front"]
        )
        out = {}
        for cam in cams:
            try:
...
class MeshGenesisSpawn(llm.Tool):
    ...
    async def async_call(
        ...
        try:
            r = await _http().post(
                f"{GENESIS_URL}/genesis/propose", json={"capability_spec": args["capability_spec"]}, headers=HEADERS
            )
            r.raise_for_status()
            return r.json()
        except Exception as e:
